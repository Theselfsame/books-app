const getData = () => {
    fetch('https://books-app-selefais.herokuapp.com/api/books')
    .then (
        res => {
            return res.json()
        }
...
const postData = (e) => {
    ...
    fetch('https://books-app-selefais.herokuapp.com/api/books', options)
    .then (
        res => {
            return res.json()
        }
...
const deleteData = (e) => {
    if(e.target.classList.contains('delete-btn')){
        ...
        fetch(`https://books-app-selefais.herokuapp.com/api/books/${bookId}`, options)
        .then(
            res => {
                return res.json();
            }
...
const editData = (e) => {
    ...
    let bookPrice = document.querySelector('#book_price_edit');
...
/*     bookItem.children[0].innerText = bookName.value;
    ...
    const options = {
        ...
        body: JSON.stringify({
            bookName: bookName.value,
            bookPrice: bookPrice.value.replace('$',""),
            bookPicture: bookImage.value
        }),
        headers: {
            'Accept': 'application/json',
    ...
    fetch(`https://books-app-selefais.herokuapp.com/api/books/${currentEditBook}`, options)
    .then (
        res => {
            return res.json()
        }
